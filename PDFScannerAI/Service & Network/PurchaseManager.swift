...
let UNLOCK_PREMIUM = "pdf_scanner_premium"
let UNLOCK_1WEEK_SUBSCRIPTION = "pdf_scanner_weekly_199t"
let UNLOCK_1MONTH_SUBSCRIPTION = "pdf_scanner_monthly_399"
let UNLOCK_1YEAR_SUBSCRIPTION = "pdf_scanner_annually_1499"
...
@objc public class PurchaseManager: NSObject {
    ...
    @objc static public let shared = PurchaseManager()
    @objc var didPurchase: ((Bool) -> Void)? = nil
    @objc var didRestore: ((Bool) -> Void)? = nil
    ...
    @objc public func isPurchased() -> Bool {
        //return true
        let purchased = UserDefaults.standard.bool(forKey: IS_PURCHASED)
        if purchased {
            if let expireDate = UserDefaults.standard.object(forKey: EXPIRE_DATE) as? Date, expireDate >= Date() {
    ...
    @objc public func isPurchased(productId: String) -> Bool {
        ...
        if UserDefaults.standard.bool(forKey: UNLOCK_PREMIUM) {
            return true
        }
    ...
    @objc public func isPurchasedOne() -> Bool {
        return isPurchased(productId: UNLOCK_PREMIUM)
    }
    ...
    @objc public func updateSubscriptionStatus(_ completion: (() -> Void)? = nil) {
    ...
    @objc public func purchase(productId: String) {
        SwiftyStoreKit.purchaseProduct(productId, atomically: true) { result in
            ...
            if case .success(let purchase) = result {
                ...
                if purchase.productId == UNLOCK_1WEEK_SUBSCRIPTION || purchase.productId == UNLOCK_1MONTH_SUBSCRIPTION || purchase.productId == UNLOCK_1YEAR_SUBSCRIPTION {
    ...
    func verifyReceipt(completion: @escaping (VerifyReceiptResult) -> Void) {
        ...
        let appleValidator = AppleReceiptValidator(service: .production, sharedSecret: "26b6417c2ca44c52a83f30e77a24c038")
