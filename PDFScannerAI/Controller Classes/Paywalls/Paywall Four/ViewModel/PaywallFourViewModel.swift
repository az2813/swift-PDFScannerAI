...
class PaywallFourViewModel {
    ...
    func fetchProducts() {
        Adapty.getPaywall(placementId: "placement_4") { [weak self] result in
    ...
    private func handle(products: [AdaptyPaywallProduct]) {
        let mapped = products.map { product -> ProductInfo in
            ...
            return ProductInfo(product: product, durationTitle: durationTitle, priceTrialLabel: priceTrialLabel, weeklyPriceLabel: weeklyPrice, priceLabel: price)
        }
