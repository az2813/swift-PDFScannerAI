...
class PaywallFourTableViewHelper: NSObject {
    private weak var tableView: UITableView?
    private let viewModel: PaywallFourViewModel
    private let interactionHelper: PaywallFourInteractionHelper

    private var tableHeaderView: PaywallFourTableHeaderView!
    var headerView: PaywallFourTableHeaderView { tableHeaderView }

    init(tableView: UITableView, viewModel: PaywallFourViewModel, interactionHelper: PaywallFourInteractionHelper) {
    ...
    private func setupTableView() {
        ...
        tableHeaderView = PaywallFourTableHeaderView(frame: CGRect(x: 0, y: 0, width: tableView.frame.width, height: 60))
...
extension PaywallFourTableViewHelper: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        viewModel.products.count
    }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "PaywallFourTableCell", for: indexPath) as? PaywallFourTableCell else {
            return UITableViewCell()
        }
        let item = viewModel.products[indexPath.row]
        cell.configure(duration: item.durationTitle, priceTrial: item.priceTrialLabel, priceOnly: item.priceLabel)
