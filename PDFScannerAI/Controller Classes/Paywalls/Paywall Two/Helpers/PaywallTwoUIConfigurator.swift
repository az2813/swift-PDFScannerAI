...
final class PaywallTwoUIConfigurator {
    func configureUI(for viewController: PaywallTwoViewController) {
    ...
    private func setupNavigationButtons(for vc: PaywallTwoViewController) {
        setupCustomButton(
            imageName: "cross_icon",
            action: #selector(PaywallTwoViewController.customBackButtonTapped),
            isRight: true,
            in: vc
        )
    }

    private func setupCustomButton(imageName: String? = nil, title: String? = nil, action: Selector, isRight: Bool, in vc: PaywallTwoViewController) {
